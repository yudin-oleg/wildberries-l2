...
export class Tetris{
    ...
    generatePlayfield(){
        this.playfield = new Array(PLAYFIELD_ROWS).fill().map(() => new Array(PLAYFIELD_COLUMS).fill(0));
    ...
    generateTetromino(){
        ...
        // номер колонки и строки с которой надо начать отрисовывать фигуры
    ...
    isOutsideOfGameBoard(row, column){
        return this.tetromino.column + column < 0 || this.tetromino.column + column >= PLAYFIELD_COLUMS || this.tetromino.row + row >= this.playfield.length;
    }
